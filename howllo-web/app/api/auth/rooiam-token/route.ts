...
export async function POST(request: NextRequest) {
  ...
  const globalBase = process.env.NEXT_PUBLIC_ROOIAM_WIDGET_BASE_URL?.trim() || "https://api.rooiam.com/login-widget";
  const globalClientId = process.env.NEXT_PUBLIC_ROOIAM_WIDGET_CLIENT_ID?.trim();
  if (!body.tenantSlug && !globalClientId) return error("RooIAM is not configured.", 503);
  ...
  if (body.tenantSlug) {
    ...
    try {
      const config = await getCustomerRooiamConfig(body.tenantSlug);
